    <template id="t_petugasList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">T Petugas</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/t_petugas/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/t_petugas">T Petugas</router-link>
                                            </li>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.kode_petugas" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td><router-link :to="'/t_petugas/view/' +  data.kode_petugas">{{data.kode_petugas}}</router-link></td>
                                                    ...
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/t_petugas/view/' + data.kode_petugas">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/t_petugas/edit/' + data.kode_petugas">
                                                            <i class="fa fa-edit"></i> 
                                                            </router-link>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.kode_petugas,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.kode_petugas"><i class="fa fa-times"></i></span>
                                                                <clip-loader :loading="deleting == data.kode_petugas" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="t_petugas/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var T_PetugasListComponent = Vue.component('t_petugasList', {
		template: '#t_petugasList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 't_petugaslist',
			},
			apipath : {
				type : String,
				default : 't_petugas/list',
			},
			tablestyle: {
				type: String,
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].kode_petugas;
